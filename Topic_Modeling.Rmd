...
The third technique will be Topic Modeling. This is a text mining technique that automatically identifies the main themes or topics that appear in a set of documents, without the need for prior labeling. It uses statistical models, such as LDA, which cluster words that often appear together and thus infer the “hidden topics” of the text. In the context of this work, applying topic modeling on president's speeches allows us to automatically discover which are the major thematic axes of their message (such as economy, health, immigration, etc.) and how they are distributed throughout the speech. This technique is particularly useful because it does not require prior knowledge of the topics discussed, which makes it a powerful exploratory tool for analyzing political speeches. The results can help to visualize which ideas predominate, whether there is a clear discursive structure and which aspects are repeated or carry more weight within the speaker's message.
...
We have used these libraries for: 
...
## Explore most common words by theme 
